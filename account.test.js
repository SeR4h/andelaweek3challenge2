...
test('returns new account number ', () => {
...
test('exits if user account number doesnt exist', () => {
    expect(() => { BankAccount.getBalance('SB1356') }).toThrowError('account doesnt exist or account is closed');
});
...
test('exits if user account is closed', () => {
    expect(() => { BankAccount.withdraw(200000, 'SB135') }).toThrowError('account doesnt exist or account closed');
});
